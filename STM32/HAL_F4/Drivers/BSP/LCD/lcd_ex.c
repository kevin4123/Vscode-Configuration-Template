...
void lcd_ex_ili9341_reginit(void)
{
    ...
    lcd_wr_data(0x48);
    lcd_wr_regno(0x3A); /* COLMOD: Pixel Format Set */
    lcd_wr_data(0x55); 	/* 16位/pixel (RGB 5-6-5) */
	 // Frame Rate Control
    lcd_wr_regno(0xB1);
    lcd_wr_data(0x00);
    lcd_wr_data(0x1A);
    ...
    lcd_wr_regno(0x2B); 
    lcd_wr_data(0x00); 
    lcd_wr_data(0x00); // 起始行
    lcd_wr_data(0x01); 
    lcd_wr_data(0x3f); // 结束行 (319)
	// Column Address Set
    lcd_wr_regno(0x2A);
    lcd_wr_data(0x00); 
    lcd_wr_data(0x00); // 起始列
    lcd_wr_data(0x00);
    lcd_wr_data(0xef); // 结束列 (239)
